    ...
    : IJwtTokenService
{
    ...
    private readonly DateTime expiryMinutes = DateTime.UtcNow;

    public JwtTokenService(IConfiguration config, ICurrentUserService currentUserService)
    {
        _config = config;
    ...
    public async Task<string> GenerateInvitationToken(Guid teamMemberId, Guid companyId)
    {
        var claims = new[]
        {
            ...
            new Claim(JwtRegisteredClaimNames.Aud, teamMemberId.ToString()),
        ...
        var token = new JwtSecurityToken(
            ...
            audience,
            claims: claims,
            expires: expiryMinutes.AddDays(7),
            signingCredentials: cred);
        return new JwtSecurityTokenHandler().WriteToken(token);
    }

    public async Task<string> GenerateToken(TeamMember teamMember)
    {
        var claims = new List<Claim>
        {
            new(JwtRegisteredClaimNames.Sub, teamMember.CompanyId.ToString()),
            new(JwtRegisteredClaimNames.Aud, teamMember.Id.ToString()),
        ...
        var token = new JwtSecurityToken(
            ...
            audience,
            claims: claims,
            expires: expiryMinutes.AddMinutes(double.Parse(_config["JwtSettings:ExpiryMinutes"] ?? "60")),
            signingCredentials: cred);
        return new JwtSecurityTokenHandler().WriteToken(token);
    }
    ...
    public ObjectToken VerifyToken(string token)
    {
        ...
        if (!Guid.TryParse(tokenSecurity?.Claims.First(claim => claim.Type == JwtRegisteredClaimNames.Aud).Value, out Guid teamMember))
